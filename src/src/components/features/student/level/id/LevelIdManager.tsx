...
import { getCourseDetailApi, checkEnrollmentApi, enrollCourseApi } from "@/src/actions/student/studentApi";
...
export default function LevelIdManager() {
    ...
    if (session?.user) {
      ...
      try {
        const [courseData, enrolled] = await Promise.all([
          ...
          checkEnrollmentApi(u.id, courseId),
        ]);
        setCourse(courseData);
        setIsEnrolled(enrolled);

        const response = await fetch(`/api/student/dashboard?studentId=${u.id}`);
        if (response.ok) {
