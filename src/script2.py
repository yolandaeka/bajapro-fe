...
target_pattern = re.compile(r'const activeClasses = classes\.map\(\(c: any, index: number\) => \{.*?return \{\s*id: c\.id.*?status: c\.isactive \? "Aktif" : "Non-Aktif",\s*\};\s*\}\);', re.DOTALL)

replacement = '''const courses = await handleFetch(${BASE_URL}/courses);
